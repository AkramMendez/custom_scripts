...
# Script for mapping paired-end reads to a reference genome using HISAT2 after ribosomal RNA removal using Ribodetector (https://github.com/hzi-bifo/RiboDetector)
# Duplicate reads are marked using Picard and resulting BAM files are filtered to keep uniquely mapping reads without duplicates and sorted using Sambamba and Samtools.
...
for read in $(ls | grep -oP "(.*_R(1|2))(?=_nonrrna_\d.fq.gz)" | sort | uniq)
do
	sample_name=$(basename $read)
	R1=$(echo "${read}"_nonrrna_1.fq.gz)
	R2=$(echo "${read}"_nonrrna_2.fq.gz)
