...
mkdir -p ${outdir}/MACS2_${sample_name}
...
awk '{if($9 >= 1.3) print }' m6a_peaks.xls > m6a_sig_peaks.xls

awk '{if($9 >= 1.3) print }' m6a_peaks.xls > m6a_negativePeaks_sig_peaks.xls
...
fastaFromBed -s -fi ${ref_genome} -bed bestPeaks.bed -fo bestPeaks.fa
fastaFromBed -s -fi ${ref_genome} -bed bestPeaks_negativePeaks.bed -fo bestPeaks_negativePeaks.fa
...
echo "Motif finding with HOMER"

findMotifsGenome.pl bestPeaks.bed ${ref_genome} homerMotifs_${sample_name} -size 75 -len 6 -basic -rna -p ${n}
...
#fastaFromBed -s -fi ${ref_genome} -bed m6a_sig_peaks_summit.bed -fo m6a_sig_peaks_summit.fa
#fastaFromBed -s -fi ${ref_genome} -bed m6a_sig_peaks_negativePeaks_summit.bed -fo m6a_sig_peaks_negativePeaks_summit.fa
