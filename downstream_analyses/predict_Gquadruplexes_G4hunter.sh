...
python2.7 G4Hunter.py -i ${fasta} -o ${outdir} -w 25 -s 1.5

echo "Predicting R-loop forming sequences coordinates in BED format"
python2.7 ${RloopFinder} --model m1 -bed -i ${fasta} -o ${name}_RloppFinder_QmRLFS.bed --log --verbose

echo "Predicting R-loop forming sequences results in tabular format"
python2.7 ${RloopFinder} --model m1 -i ${fasta} -o ${name}_RloppFinder_QmRLFS_table.txt --log --verbose
